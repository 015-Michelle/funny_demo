...
class ReorderableGridView extends StatefulWidget {
  ...
  ReorderableGridView({
    ...
    @Deprecated("Not used any more, because always anti multiDrag now.") this.antiMultiDrag = false,
  }) : super(key: key);
...
class _ReorderableGridViewState extends State<ReorderableGridView>
  ...
  void startDragRecognizer(
      int index, PointerDownEvent event, MultiDragGestureRecognizer recognizer) {
  ...
  Offset getPos(int index, {bool safe = true}) {
    ...
    double itemWidth =
        (renderBox.size.width - (widget.crossAxisCount - 1) * widget.crossAxisSpacing) /
  ...
  Widget build(BuildContext context) {
    ...
    //TODO:TTM try to modify GridView to build different item
...
class _ReorderableGridItem extends StatefulWidget {
  ...
  const _ReorderableGridItem(
      {required this.child, required this.key, required this.index, required this.capturedThemes})
      : super(key: key);
...
class _ReorderableGridItemState extends State<_ReorderableGridItem> with TickerProviderStateMixin {
  ...
  Offset get offset {
    if (_offsetAnimation != null) {
      return Offset.lerp(
          _startOffset, _targetOffset, Curves.easeInOut.transform(_offsetAnimation!.value))!;
    }
    ...
    if (!_autoScrolling) {
      ...
      if (needScroll) {
        ...
        await position.animateTo(newOffset!,
            duration: Duration(milliseconds: _DEFAULT_SCROLL_DURATION), curve: Curves.linear);
